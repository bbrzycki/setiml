...
for SNR in SNRs:
    dirs = ['/datax/scratch/bbrzycki/data/%s/%s/train/%s/' % (dir, SNR, label) for label in labels] \
            + ['/datax/scratch/bbrzycki/data/%s/%s/validation/%s/' % (dir, SNR, label) for label in labels] 
...
for SNR in SNRs:
    for name, num in datasets:
        ...
        for i in range(num):
            ...
            output_fn = '/datax/scratch/bbrzycki/data/%s/%s/%s/%s/%s_%04d.png' % (dir,SNR,name,'present','present',i)
